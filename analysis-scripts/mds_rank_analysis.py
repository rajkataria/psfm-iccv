...
def plot_mds_rank_vs_gt_ranks(datasets, options={}):
    ...
    for i,t in enumerate(datasets):
        ...
        for im1 in im_closest_images_gt:
            for im2 in im_closest_images_gt[im1]:

                try:
                    distance_rank_percentage_im1_im2_gt = 100.0 * im_closest_images_gt[im1].index(im2) / len(data.images())
                    mds_rank_percentage_im1_im2 = 100.0 * im_closest_images[im1].index(im2) / len(data.images())
